...
export const exportToExcelCompatibleCSV = async (language: 'en' | 'am' = 'en') => {
  try {
    ...
    // Create comprehensive CSV content with multiple sheets as sections
    let csvContent = '';
    ...
    // 2. Dimension Analysis Section
    csvContent += createDimensionSection(summaryData, responses, language);
    csvContent += '\n\n';
    
    // 3. Question Performance Section
    ...
    // 6. Font Instructions (for Amharic)
    if (language === 'am') {
...
const createDimensionSection = (_summaryData: any, responses: any[], language: 'en' | 'am'): string => {
...
const createDemographicsSection = (summaryData: any, language: 'en' | 'am'): string => {
  ...
  // Gender Distribution
  section += `${escapeCSV(language === 'am' ? 'የፆታ ስርጭት' : 'Gender Distribution')}\n`;
  ...
  // Age Distribution
  section += `${escapeCSV(language === 'am' ? 'የዕድሜ ስርጭት' : 'Age Distribution')}\n`;
  section += `18-30,${escapeCSV(summaryData.demographicCounts?.age?.['18-30'] || 0)}\n`;
  section += `31-40,${escapeCSV(summaryData.demographicCounts?.age?.['31-40'] || 0)}\n`;
  section += `41-50,${escapeCSV(summaryData.demographicCounts?.age?.['41-50'] || 0)}\n`;
  section += `50+,${escapeCSV(summaryData.demographicCounts?.age?.['50+'] || 0)}\n`;
  section += '\n';
  
  // Education Distribution
  section += `${escapeCSV(language === 'am' ? 'የትምህርት ደረጃ ስርጭት' : 'Education Level Distribution')}\n`;
  section += `${escapeCSV(language === 'am' ? 'ሰርተፊኬት' : 'Certificate')},${escapeCSV(summaryData.demographicCounts?.educationLevel?.certificate || 0)}\n`;
  section += `${escapeCSV(language === 'am' ? 'ዲፕሎማ' : 'Diploma')},${escapeCSV(summaryData.demographicCounts?.educationLevel?.diploma || 0)}\n`;
  section += `${escapeCSV(language === 'am' ? 'የመጀመሪያ ዲግሪ' : 'First Degree')},${escapeCSV(summaryData.demographicCounts?.educationLevel?.first_degree || 0)}\n`;
...
const createFontInstructionsSection = (): string => {
  ...
  section += '\n';
  section += 'ይህ ሪፖርት አማርኛ ጽሁፍ ይዟል። ትክክለኛ እይታ ለማግኘት የሚከተሉትን ያድርጉ:\n';
  section += 'This report contains Amharic text. For proper display please follow these steps:\n';
  section += '\n';
  section += '1. የሚመከሩ ፊደሎች - Recommended Fonts:\n';
  section += '   • Nyala (Windows ላይ ነባር - Built-in on Windows)\n';
  section += '   • Ebrima (Windows 8+ ላይ ነባር - Built-in on Windows 8+)\n';
  section += '   • Noto Sans Ethiopic (Google Fonts)\n';
  section += '\n';
  section += '2. Excel ውስጥ ፊደል መቀየር - Changing Font in Excel:\n';
  section += '   • ሁሉንም ሴሎች ይምረጡ (Ctrl+A)\n';
  section += '   • Home > Font dropdown\n';
  section += '   • ከላይ የተዘረዘሩትን ፊደሎች ይምረጡ\n';
